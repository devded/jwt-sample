...
def check_math_capabilities(config, ext, moredefs, mathlibs):
    ...
    for dec, fn in OPTIONAL_FUNCTION_ATTRIBUTES:
        if config.check_gcc_function_attribute(dec, fn):
            ...
            if fn == 'attribute_target_avx512f' and (
                sys.platform in ('win32', 'cygwin')
                and config.check_compiler_gcc()
                and not config.check_gcc_version_at_least(8, 4)
            ):
                ext.extra_compile_args.extend([f'-ffixed-xmm{n}' for n in range(16, 32)])
...
def check_complex(config, mathlibs):
    ...
    if st:
        ...
        for t in C99_COMPLEX_TYPES:
            ...
            if st:
                pub.append((f'NPY_HAVE_{type2def(t)}', 1))
